...
private:
  ...
  requires (!is_value_token<type> ()) void addToken ()
  {
  ...
  template <TokenType type>
  requires (type == TokenType::TOKEN_NUMBER) void addToken (double value)
  {
  ...
  template <TokenType type>
  requires (type
            == TokenType::TOKEN_STRING) void addToken (std::string_view value)
  {
    m_tokens.emplace_back (std::make_unique<ValueToken<type> > (
        value, m_pos_start, m_pos_current - m_pos_start, m_line_start));
  }
  ...
  template <TokenType type>
  requires (type == TokenType::TOKEN_IDENTIFIER) void addToken (
