...
Environment::EnvironmentException::EnvironmentException (
...
void
Environment::define (std::string_view name,
...
void
Environment::assign (Box<AssignExpression> const &expr,
...
auto
Environment::get (Box<VariableExpression> const &expr)
...
auto
Environment::error (std::optional<Token const *> token, std::string const &msg)
    -> EnvironmentException
{
  ...
  m_error_reporter.setError ("Environment", str);
  return { str };
}