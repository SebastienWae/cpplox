...
TEST_CASE ("lexer")
{
  ...
  SUBCASE ("empty string")
  {
    ...
    CHECK (static_cast<ValueToken<TokenType::TOKEN_STRING> *> (
               tokens.at (0).get ())
               ->getValue ()
           == "");
    CHECK (!er.hasError ());
  ...
  SUBCASE ("numbers literal")
  {
    ...
    CHECK (static_cast<ValueToken<TokenType::TOKEN_NUMBER> *> (
               tokens.at (0).get ())
               ->getValue ()
           == 123.);
  ...
  {
    Lexer l ("and class else false fun for if nil or print return super this "
             "true var while",
             er);
