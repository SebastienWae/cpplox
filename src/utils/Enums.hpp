...
{
constexpr auto
is_pretty (char ch) noexcept
{
  return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch == '_';
}

constexpr auto
pretty_name (std::string_view sv) noexcept
{
...
constexpr auto
count_values (const bool (&valid)[N])
{
  std::size_t count = 0;
...
constexpr auto
values (std::index_sequence<I...>) noexcept
{
...
template <typename E>
constexpr auto
enum_to_string (E value) -> std::string_view
{
